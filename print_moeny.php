...
<body>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            $sql = "SELECT  *  FROM tb_booking_detail a INNER JOIN tb_booking b ON a.booking_id = b.booking_id INNER JOIN tb_room d ON a.room_id = d.room_id INNER JOIN tb_homestay m ON d.homestay_id = m.homestay_id  INNER JOIN tb_payment t ON b.booking_id = t.booking_id WHERE  booking_detail_status BETWEEN 3 AND 5";
            $result = result_array($sql);

            $sql2 = "SELECT * FROM tb_ledger WHERE ledger_type = 1";
            
            ?>
             <?php foreach ($result as $key => $value) { ?>
                <tr>
                    <td class="center"><?= $key + 1; ?></td>
                    <td class="center"><?= $value['payment_date']; ?></td>
                    <td class="center"><?= $value['homestay_name']; ?></td>
                    <td class="center"><?= $value['payment_money']==ledger_type($num=1);?></td>
                    <td class="center"><?= ledger_type($num=2); ?></td>
