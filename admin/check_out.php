    ...
    <div class="col-sm-9 col-lg-9">
        <div id="register-wraper">
            <div id="register-form" style="position: relative;">
                ...
                <legend>เช็คเอาท์ออกห้องพัก</legend>

                <?PHP
                $uid = check_session("id");
                $sql = "SELECT * FROM tb_booking a INNER JOIN tb_booking_detail b ON a.booking_id = b.booking_id INNER JOIN tb_room d ON b.room_id = d.room_id INNER JOIN tb_homestay e ON d.homestay_id = e.homestay_id INNER JOIN tb_user g ON a.user_id = g.user_id WHERE booking_status = 3 AND booking_detail_status = 4 AND e.user_id = '{$uid}'";
                        ...
                        <?PHP foreach ($result as $key => $row) { ?>
                            <tr>
                                ...
                                <td class="center">
                                    ...
                                    <a href="process/booking_detail_update.php?status=5&id=<?= $row['booking_detail_id']; ?>&url=check_out.php"
                                       class="btn btn-warning btn-rounded btn-sm"
                                       onclick="return confirm_custom(this.href,'ยืนยันการเช็คเอาท์?');">
                                        เช็คเอาท์
                                    </a>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td>
                                    <ul class="list-group">
                                        ...
                                        <li class="list-group-item">
                                            <b>สถานะ : <?= booking_status($row['booking_status']); ?></b>
                                        </li>
                                        <li class="list-group-item">
                                            <a target="_blank" href="../print_booking.php?print&id=<?= $row['booking_id']; ?>" class="btn btn-info btn-rounded btn-sm mb-2">
                                                พิมพ์
                                            </a>
                                            <a href="process/booking_detail_update.php?status=5&id=<?= $row['booking_detail_id']; ?>&url=check_out.php" class="btn btn-warning btn-rounded btn-sm" onclick="return confirm_custom(this.href,'ยืนยันการเช็คเอาท์?');">
                                                เช็คเอาท์
                                            </a>
                                        </li>
                                    </ul>
